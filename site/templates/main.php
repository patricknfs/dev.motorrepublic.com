    ...
    <header data-sticky data-margin-top="0">
      ...
      <div id="logorow" class="grid-x align-justify">
        ...
        <div class="cell small-12 medium-3" id="header_tel">
          <h2>Call: <a href="tel:00441217949073">0121 794 9073</a></h2>
        </div>
      </div>
      <section id="menurow">
      ...
      function populate(s1,s2){
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          echo "else if(s1.value == '" . $row['manufacturer'] . "'){
            var optionArray = ['|',";
            $query2 = "SELECT DISTINCT(`model`) FROM `team`.`rates_combined` WHERE `manufacturer` = '" . $row['manufacturer'] . "' AND `special` = 1 ORDER BY `model` ASC";
            // echo $query2;
