...
if( isset($_POST['manufacturer']) ) {
  ...
  $total_rows = $countres->num_rows;
  $total_pages = ceil($total_rows / $no_of_records_per_page);
  $query = "SELECT `id`,`cap_id`,`cap_code`,`source`,`manufacturer`,`model`,`descr`,`term`,`mileage`,min(`rental`) AS `rental`,`vehicle_list_price`,`vehicle_otr_price`,`p11d_price`,`CO2`,`deal_notes` FROM `team`.`rates_combined` GROUP BY `cap_id` ORDER BY `rental` DESC LIMIT $offset, $no_of_records_per_page";
}
