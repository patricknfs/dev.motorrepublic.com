...
try
{
  ...
  if(null !== $input->urlSegment()) {
    $query = "SELECT `id`,`cap_id`,`cap_code`,`src`,`manufacturer`,`model`,`descr`,`term`,`mileage`,min(`rental`) AS `rental`,`vehicle_list_price`,`vehicle_otr_price`,`p11d_price`,`CO2`,`lcv`,`upfront`, `special`, `special_upfront`, `website_deal_notes` FROM `team`.`rates_combined_terse_copy` WHERE `cap_id` = " . $input->urlSegment() . " ORDER BY `rental` ASC LIMIT 1";
    // echo $query;
    ...
    if($data['special'] == 1){
      ...
      $pch_initial = (($data['special_upfront'])?number_format(($data['special_upfront'] * 1.2), 2, '.', ','):number_format((($data['rental'] * $data['upfront'])*1.2), 2, '.', ','));
    }
    else {
